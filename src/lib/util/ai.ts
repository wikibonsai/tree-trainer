...
import { IGNORE_LAZY, SYSTEM_PROMPT, TAKE_TIME, PEPTALK } from './prompt';
...
export async function trainTree(userMessage: string, opts: Opts): Promise<string> {
  console.log('tree-training with opts: ', opts);
  try {
    validate(opts.llm);
  } catch (e) {
